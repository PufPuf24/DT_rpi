...
class MonitorApp:
    ...
    def _startFmuThermal(self):
        # Mid pack (CH1, the first configured resistance channel), not
        # self.ecmTempSourceIndex (T3 by default) -- the pack thermal model's
        # 24-cell ROM wants a single BULK initial condition, best represented by
        # the sensor placed in the middle of the pack, not whichever single-point
        # sensor the ECM's own electrical-parameter lookup happens to use. Takes
        # one direct synchronous reading (same pattern as
        # _estimateInitialSocFromLiveReading, called just before this in
        # toggleMeasurement) because self.resY is still empty here -- the worker
        # thread that would otherwise fill it hasn't started yet, so reading
        # self.resY at this point always silently fell back to the hardcoded
        # 25.0 C default. Falls back to that same default if the reading doesn't
        # look valid -- per config.py's own note, Mid pack is often not
        # physically wired on the real bench.
        initialTempC = 25.0
        midPackCh, midPackName = self.resistChannels[0]
        speed = "Fast" if self.pollPeriod < SLOW_THRESHOLD else "Slow"
